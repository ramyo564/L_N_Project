...
- **왜 선택했는가**: AWS 프리티어 환경에서는 메모리/성능 제약이 커서 실서비스 부하 검증에 한계가 있었고, 홈서버가 비용 대비 성능에서 유리했다.
- **안 고른 대안**: AWS 유료 운영(RDS 포함) 지속 사용.
- **대안 대비 손해(Trade-off)**: AWS 관리형 서비스(RDS 등)에서 얻는 실무형 운영 경험이 줄고, 인프라 운영 책임을 직접 가져가야 한다.
- **손해 감수 근거(제약/운영조건)**: CI/CD 자동배포, VPN/터널 기반 보안, 모니터링/알림 체계를 구축한 상태에서 초기 단계에는 고정비 최소화가 더 중요하다고 판단했다.
- **운영 원칙**: 현재는 비용 효율을 우선해 홈서버 + Cloudflare/WireGuard 조합으로 운영하고, 관측 체계와 자동배포로 안정성을 보완한다.
